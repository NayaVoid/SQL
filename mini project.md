...
### 2 Определили пользователя, который за весь период посмотрел наибольшее количество обьявлении
```sql
...
### 3.1 Сравнили зарплаты каждого сотрудника с средней зарплатой в их отделе:
   ...
   limit 100;
   ```
   ### 3.2 Ранжируем сотрудников в отделе по стажу работы:
  ```sql
...
LIMIT 100;
```
### 3.3 Определяем начальной и последней зарплаты сотрудника:
