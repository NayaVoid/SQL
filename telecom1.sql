...
#Проект: Анализ и прогнозирование оттока клиентов телеком-оператора.Результаты в SQL: Провела финансовый анализ базы данных (7043 строки). Выявила, что компания ежемесячно теряет 30% выручки (139k) из-за оттока клиентов.Результаты в Python: Построила модель машинного обучения (Random Forest), способную предсказывать уход клиентов с точностью 63% и полнотой 51%.Инсайт: Самой критичной зоной риска определены новые клиенты на помесячной оплате с оптоволоконным интернетом (Fiber optic).
...
#Когда вы досчитаете эти метрики, ваш проект будет полностью готов. Лучший способ презентовать его работодателю — это объединить всё в один файл Jupyter Notebook (или выложить на GitHub) по следующей структуре:Бизнес-задача: Снизить отток клиентов в телекоме.SQL-этап: Выгрузка данных, расчет LTV, определение финансовых потерь и когортный анализ.Python-этап: Визуализация найденных в SQL аномалий (красивые графики) и обучение ML-модели для прогнозирования.Бизнес-рекомендации: Что конкретно вы предлагаете сделать директору (например: «Перевести клиентов с Fiber Optic на годовые контракты, предложив скидку, так как это снизит отток на Х%»).
